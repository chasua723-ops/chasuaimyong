...
const CHOICE_QUESTION_STYLES = [
  ...
  '문장 속 특정 부분(밑줄 친 부분)에 대해 묻는 형식 (어법 오류, 용법, 의미 등 — 그 부분을 반드시 prompt 텍스트 안에서 정확히 [[ ]]로 감싸서 표시하세요. 예: "다음 문장에서 [[这个]]의 용법으로 옳은 것은?")',
...
export async function generateQuestions(
  ...
  const raw = await askClaude(client, prompt, {
    system:
      ...
      'prompt 텍스트는 플레인 텍스트로만 렌더링되므로, HTML 태그(<u> 등)나 마크다운은 절대 쓰지 마세요. ' +
      '문장 속 특정 부분을 강조하거나 지칭해야 한다면 그 부분만 정확히 [[ ]]로 감싸서 표시하세요.',
    maxTokens: 2048,
  });
