...
describe('generateEssayPractice', () => {
  ...
  it('retries on a different page when generateQuestions rejects (e.g. the page has no teachable content)', async () => {
    // Reproduces the same live failure hit by 더 풀기: a random page landed on front matter
    // (copyright/ISBN info) and Claude declined with prose instead of JSON, which
    // generateQuestions surfaces as a thrown error rather than a missing book_pages row.
    vi.mocked(generateQuestions)
      .mockRejectedValueOnce(new Error('Failed to parse JSON from Claude response: 이 내용으로는...'))
      .mockResolvedValueOnce([
        { type: 'essay', sourcePage: 5, prompt: '재시도 성공 서술형 문제', correctAnswer: '' },
      ]);
    const supabase = createMockSupabase(baseTables());
    const randomSpy = vi.spyOn(Math, 'random');
    ...
    expect(result.prompt).toBe('재시도 성공 서술형 문제');
    expect(result.sourcePage).toBe(10);
    randomSpy.mockRestore();
  });
});
